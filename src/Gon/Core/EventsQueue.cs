...
namespace Gon
{
...
#endif
    {
        ...
        public bool DetectIntersection(LeftEvent<Scalar> belowEvent, LeftEvent<Scalar> event_)
        {
            ...
            var eventStartOrientation = Orienteer<Scalar>.Orient(
                belowEventStart,
                belowEventEnd,
                eventStart
            );
            var eventEndOrientation = Orienteer<Scalar>.Orient(
                belowEventStart,
                belowEventEnd,
                eventEnd
            );
            ...
            )
            {
                ...
                {
                    var belowEventStartOrientation = Orienteer<Scalar>.Orient(
                        eventStart,
                        eventEnd,
                        belowEventStart
                    );
                    var belowEventEndOrientation = Orienteer<Scalar>.Orient(
                        eventStart,
                        eventEnd,
                        belowEventEnd
                    );
            ...
            else
            {
                ...
                if (eventStart == belowEventStart)
                {
                    if (eventEnd != belowEventEnd)
                    {
                        var (maxEndEvent, minEndEvent) = (
                            eventEnd < belowEventEnd ? (belowEvent, event_) : (event_, belowEvent)
                        );
                ...
                else if (eventEnd == belowEventEnd)
                {
                    var (maxStartEvent, minStartEvent) = (
                        eventStart < belowEventStart ? (belowEvent, event_) : (event_, belowEvent)
                    );
        ...
        {
            var scale = ToSegmentsIntersectionScale(firstStart, firstEnd, secondStart, secondEnd);
#if NET7_0_OR_GREATER
        ...
        )
        {
            ...
            dynamic
#endif
            dividend = CrossMultiplier<Scalar>.CrossMultiply(
                firstStart,
                secondStart,
                secondStart,
                secondEnd
            ...
            dynamic
#endif
            divisor = CrossMultiplier<Scalar>.CrossMultiply(
                firstStart,
                firstEnd,
                secondStart,
                secondEnd
