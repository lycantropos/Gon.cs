...
@given(strategies.segments)
def test_coordinates_reversals(segment: Segment) -> None:
    assert (reverse_box_coordinates(segment.bounding_box)
            == reverse_segment_coordinates(segment).bounding_box)


@given(strategies.segments)
def test_vertices_reversals(segment: Segment) -> None:
    assert (segment.bounding_box
            == reverse_segment_endpoints(segment).bounding_box)
